...
function StormUI:CreateWindow(config)
    ...
    local height = config.Height or 290
    ...
    -- O overlay cobre o Main inteiro com fundo semi-transparente.
    -- Todos os AddDialog da janela reutilizam este mesmo modal.
    ...
    -- UICorner no overlay para não ultrapassar os cantos arredondados do Main
    Corner(DialogOverlay, 12)
    ...
    -- Barra de acento no topo do modal
    local DialogTopBar = Create("Frame", {
    ...
    Corner(DialogTopBar, 3)
    Create("Frame", { -- quadra cantos inferiores da barra
        Position = UDim2.new(0, 0, 0.5, 0),
        Size = UDim2.new(1, 0, 0.5, 0),
        BackgroundColor3 = Theme.Accent,
        BorderSizePixel = 0,
    ...
    Gradient(DialogTopBar, Theme.AccentDark, Theme.AccentLight)
    ...
    local DialogInnerLayout = ListLayout(DialogInner, Enum.FillDirection.Vertical, 10)
    DialogInnerLayout.HorizontalAlignment = Enum.HorizontalAlignment.Center
    ...
    -- Linha de botões
    local DialogBtnRow = Create("Frame", {
        Size = UDim2.new(1, 0, 0, 34),
        BackgroundTransparency = 1,
        ZIndex = 81,
        Parent = DialogInner,
    })
    local BtnRowLayout = ListLayout(DialogBtnRow, Enum.FillDirection.Horizontal, 8)
    ...
    Corner(DialogYesBtn, 7)
    Gradient(DialogYesBtn, Theme.AccentDark, Theme.AccentLight)

    -- Callback atual do botão Sim (trocado a cada OpenDialog)
    ...
    -- Botão Sim: fecha e executa o callback configurado
    ...
    -- Fechar ao clicar no overlay (fora da DialogBox)
    local DialogOverlayBtn = Create("TextButton", {
        Size = UDim2.new(1, 0, 1, 0),
        BackgroundTransparency = 1,
        Text = "",
        ZIndex = 80,
    ...
    DialogOverlayBtn.MouseButton1Click:Connect(function()
    ...
    function Window:AddTab(tabConfig)
        ...
        function Tab:AddSection(sectionConfig)
            ...
            function Section:AddToggle(cfg)
                ...
                -- Com descrição: nome no topo; sem: centralizado verticalmente
                Create("TextLabel", {
                    AnchorPoint = Vector2.new(0, hasDesc and 0 or 0.5),
                    Position = hasDesc and UDim2.new(0, 10, 0, 9) or UDim2.new(0, 10, 0.5, 0),
            ...
            function Section:AddSlider(cfg)
                ...
                local Container = Create("Frame", {
                    Size = UDim2.new(1, 0, 0, hasDesc and 66 or 52),
                    BackgroundColor3 = Theme.Element,
                    BorderSizePixel = 0,
                    LayoutOrder = NextOrder(),
                    ZIndex = 3,
            ...
            function Section:AddColorPicker(cfg)
                ...
                local Popup = Create("Frame", {
                    ...
                    BorderSizePixel = 0,
                    ZIndex = 501,
                    -- ClipsDescendants = true é obrigatório aqui.
                    -- Com false, ao fechar (tween height→0) os filhos
                    -- continuam visíveis fora do popup, causando visual bugado.
                    ClipsDescendants = true,
                    Visible = false,
                    Parent = Overlay,
                })
                ...
                local popOpen = false
                -- Largura salva ao abrir. Lemos Container.AbsoluteSize.X uma vez
                -- e reutilizamos ao fechar, pois Popup.AbsoluteSize.X pode
                -- retornar 0 se a leitura ocorrer no meio de um tween ativo.
                local popupW = 0
            ...
            -- Dialog
            -- Cria um botão que, ao ser clicado, abre o modal de confirmação.
            -- OpenDialog/CloseDialog são capturados do escopo da janela (closure).
