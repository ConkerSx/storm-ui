--[[
    ╔══════════════════════════════════════════════╗
    ║         STORM HUB UI LIBRARY  v3.0           ║
    ╠══════════════════════════════════════════════╣
    ║  Correções v3.0:                             ║
    ║  1. Drag sem teleporte (converte pra offset) ║
    ║  2. ColorPicker: não fecha ao arrastar HSV   ║
    ║     + fecha ao clicar fora da interface      ║
    ║  3. Cantos arredondados reais (Corner Body)  ║
    ║  4. Minimizar: só esconde conteúdo           ║
    ║  5. Botão flutuante com fade in/out          ║
    ║  6. Sistema de temas: Dark / Purple / Green  ║
    ╚══════════════════════════════════════════════╝

    USO RÁPIDO:
        StormHub:SetTheme("Green")          -- antes de criar a janela
        local W = StormHub:CreateWindow({...})
        local T = W:AddTab({Name="Main"})
        T:AddToggle({Name="X", Callback=...})   -- direto na tab
        local S = T:AddSection({Name="Combat"}) -- ou com section
        S:AddButton({...})
        W:AddFloatButton({...})             -- botão flutuante opcional
--]]
...
local Themes = {
    ...
    Dark = {
        ...
        StrokeAccent= Color3.fromRGB(80,  80,  80 ),
        TextAccent  = Color3.fromRGB(160, 160, 160),
    },
    Green = {
        Accent      = Color3.fromRGB(40,  180, 70 ),
        AccentDark  = Color3.fromRGB(28,  130, 50 ),
        AccentLight = Color3.fromRGB(80,  220, 110),
        TabActive   = Color3.fromRGB(40,  180, 70 ),
        ToggleOn    = Color3.fromRGB(40,  180, 70 ),
        SliderFill  = Color3.fromRGB(40,  180, 70 ),
        StrokeAccent= Color3.fromRGB(40,  180, 70 ),
        TextAccent  = Color3.fromRGB(90,  220, 110),
    },
}
...
-- ─── [Fix 1] Dragging sem teleporte ───────────────────────────────────────────
-- Antes de registrar startPos, converte a posição do frame para offset absoluto.
-- Isso evita o teleporte que ocorre quando a posição usa escala (0.5, -width/2).
local function MakeDraggable(handle, frame)
    ...
    handle.InputBegan:Connect(function(input)
        ...
        or input.UserInputType == Enum.UserInputType.Touch then
            ...
            -- Converter para offset absoluto antes de começar o drag
...
StormHub.Themes = Themes

-- ─── SetTheme ─────────────────────────────────────────────────────────────────
function StormHub:SetTheme(name)
    local preset = Themes[name]
    if not preset then
        warn("StormHub: Tema '" .. tostring(name) .. "' inválido. Use: Dark, Purple, Green")
        return
    end
...
function StormHub:CreateWindow(config)
    ...
    -- Overlay para dropdowns / colorpicker (sem clipping, ZIndex alto)
    ...
    -- ── [Fix 3] Janela principal ───────────────────────────────────────────────
    -- ClipsDescendants = true + Corner garante cantos arredondados reais em
    -- TODOS os filhos, incluindo a parte inferior da interface.
    ...
    -- Sombra (também serve como borda visual)
    Create("ImageLabel", {
    ...
    -- ── Body ──────────────────────────────────────────────────────────────────
    -- [Fix 3] Corner no Body garante que Sidebar e ContentArea
    -- (que têm fundo preto) também tenham cantos arredondados.
    ...
    -- Sidebar (Frame das Abas — esquerda)
    ...
    -- Área de conteúdo (Frame das Funções — direita)
    ...
    local DLy = ListLayout(DialogInner, Enum.FillDirection.Vertical, 10)
    ...
    -- ── [Fix 4] Minimizar — apenas esconde o conteúdo ─────────────────────────
    -- TopBar permanece visível. Apenas Body e TopDivider desaparecem.
    ...
    -- MakeComponents é chamada por Tab (direto) e Section (dentro de seção).
    -- Todos os métodos Add* ficam disponíveis em ambos.
    local function MakeComponents(container)
        local obj   = {}
        local order = 0
        local function N() order = order + 1; return order end

        -- Helper: cria área de texto (nome + desc) que expande sozinha
        ...
        function obj:AddSlider(cfg)
            ...
            local function Update(ix)
                ...
                TKnob.Position= UDim2.new(pct,0,0.5,0)
            ...
            local function BuildList()
                ...
                for _,opt in ipairs(options) do
                    ...
                    Opt.MouseButton1Click:Connect(function()
                        selected=opt; if flag then StormHub.Flags[flag]=selected end
        ...
        function obj:AddMultiDropdown(cfg)
            ...
            UserInputService.InputBegan:Connect(function(input)
                ...
                if not ins(List) and not ins(Container) then CloseMulti() end
            end)

            local CBtn=Create("TextButton",{Size=UDim2.new(1,0,1,0),BackgroundTransparency=1,Text="",ZIndex=5,Parent=Container})
            CBtn.MouseButton1Click:Connect(function() if listOpen then CloseMulti() else OpenMulti() end end)
            CBtn.MouseEnter:Connect(function() Tween(Container,0.15,{BackgroundColor3=Theme.ElementHover}) end)
            CBtn.MouseLeave:Connect(function() Tween(Container,0.15,{BackgroundColor3=Theme.Element}) end)
        ...
        function obj:AddKeybind(cfg)
            ...
            if flag then StormHub.Flags[flag]=curKey end
            ...
            UserInputService.InputBegan:Connect(function(input,gpe)
                ...
                if listening then
                    if input.UserInputType==Enum.UserInputType.Keyboard then
                        curKey=input.KeyCode; if flag then StormHub.Flags[flag]=curKey end
                        SetL(false); callback(curKey)
                    end
                else
                    if input.KeyCode==curKey then callback(curKey) end
                end
            end)
            local K={}
            function K:Set(k) curKey=k; if flag then StormHub.Flags[flag]=k end; KeyLbl.Text=KStr(k) end
        ...
        function obj:AddTextBox(cfg)
            ...
            TB.Focused:Connect(function()
                Tween(IStroke,0.2,{Color=Theme.Accent})
                Tween(IBg,0.2,{BackgroundColor3=Color3.fromRGB(55,28,80)})
            end)
            TB.FocusLost:Connect(function(enter)
                Tween(IStroke,0.2,{Color=Theme.Stroke})
                Tween(IBg,0.2,{BackgroundColor3=Color3.fromRGB(78,78,78)})
                local val=TB.Text
                if numeric then val=tonumber(val) or tonumber(default) or 0; TB.Text=tostring(val) end
                if flag then StormHub.Flags[flag]=val end; callback(val,enter)
            end)
            local TB2={}
        ...
        -- ── [Fix 2] ColorPicker ───────────────────────────────────────────────
        -- Correções:
        --   a) hsvDragging: enquanto arrasta o seletor, não fecha o popup.
        --   b) CBtn abre o popup (só se fechado). Fecha somente via clique fora.
        --   c) Clique fora da interface fecha corretamente.
        function obj:AddColorPicker(cfg)
            ...
            if flag then StormHub.Flags[flag]=curColor end
            ...
            local hTr,hKn = MakeSlider("Hue",       0, h)
            local sTr,sKn = MakeSlider("Saturation",30, s)
            local vTr,vKn = MakeSlider("Brightness", 60, v)
            ...
            sG.Color=ColorSequence.new(Color3.fromHSV(h,0,1),Color3.fromHSV(h,1,1)); sG.Parent=sTr
            local vG=Instance.new("UIGradient")
            vG.Color=ColorSequence.new(Color3.fromHSV(h,s,0),Color3.fromHSV(h,s,1)); vG.Parent=vTr
            ...
            local function Apply()
                ...
                if flag then StormHub.Flags[flag]=curColor end; callback(curColor)
            end

            -- [Fix 2a] hsvDragging — impede fechar popup ao arrastar HSV
            local hsvDragging = false

            local function MakeTI(track,knob,onVal)
                local drag=false
                track.InputBegan:Connect(function(i)
                    if i.UserInputType==Enum.UserInputType.MouseButton1 or i.UserInputType==Enum.UserInputType.Touch then
            ...
            MakeTI(hTr,hKn,function(val) h=val end)
            MakeTI(sTr,sKn,function(val) s=val end)
            MakeTI(vTr,vKn,function(val) v=val end)

            local popOpen=false
            local popupW=0

            -- [Fix 2b] Fechar apenas via clique fora, com guard de HSV drag
            UserInputService.InputBegan:Connect(function(input)
                if not popOpen then return end
                if hsvDragging then return end  -- nunca fecha enquanto arrasta HSV
                if input.UserInputType~=Enum.UserInputType.MouseButton1
                and input.UserInputType~=Enum.UserInputType.Touch then return end
                local mp=input.Position
                local function ins(f)
                    ...
                    return mp.X>=a.X and mp.X<=a.X+sz.X and mp.Y>=a.Y and mp.Y<=a.Y+sz.Y
                end
            ...
            -- [Fix 2c] CBtn só abre o popup (não toggle), fecha via clique fora
            local CBtn=Create("TextButton",{Size=UDim2.new(1,0,1,0),BackgroundTransparency=1,Text="",ZIndex=5,Parent=Container})
            CBtn.MouseButton1Click:Connect(function()
                if popOpen then return end  -- já aberto, não faz nada
                popOpen=true
                local ap,as=Container.AbsolutePosition,Container.AbsoluteSize
                popupW=as.X
                Popup.Position=UDim2.new(0,ap.X,0,ap.Y+as.Y+4)
                Popup.Size=UDim2.new(0,popupW,0,0); Popup.Visible=true
            ...
            function CP:Set(color)
                ...
                hKn.Position=UDim2.new(h,0,0.5,0)
                sKn.Position=UDim2.new(s,0,0.5,0)
                vKn.Position=UDim2.new(v,0,0.5,0)
                Preview.BackgroundColor3=color
                if flag then StormHub.Flags[flag]=color end; Apply()
            end
        ...
        function obj:AddLabel(cfg)
            ...
            local Lbl=Create("TextLabel",{Size=UDim2.new(1,0,0,0),AutomaticSize=Enum.AutomaticSize.Y,
                ...
                TextWrapped=true,LayoutOrder=N(),ZIndex=3,Parent=container})
            Padding(Lbl,2,2,6,6)
            local L={}
    ...
    -- ── [Fix 5] AddFloatButton ─────────────────────────────────────────────────
    -- Botão flutuante arrastável fora da interface.
    -- Clique: Fade Out / Fade In da janela principal.
    function Window:AddFloatButton(cfg)
        ...
        -- Frame de fade (cobre o Main para animar entrada/saída)
        ...
        -- Dragging no botão flutuante (também usa a correção de offset)
    ...
    function Window:AddTab(tabConfig)
        ...
        -- Tab herda todos os componentes diretamente (Section opcional)
        local Tab = MakeComponents(TabPage)
        table.insert(tabs, Tab)

        function Tab:Select() ActivateTab() end

        -- AddSection é opcional — Tab pode usar com ou sem
        function Tab:AddSection(sectionConfig)
            ...
            Corner(SH, 8)
            Create("Frame", {Position=UDim2.new(0,0,0.5,0),Size=UDim2.new(1,0,0.5,0),
            ...
            local Section = MakeComponents(SC)
            return Section
        end
...
--[[
╔═══════════════════════════════════════════════════════════╗
║            STORM HUB v3.0 — EXEMPLO COMPLETO             ║
╠═══════════════════════════════════════════════════════════╣

-- Definir tema ANTES de criar a janela
StormHub:SetTheme("Green")   -- Dark | Purple | Green
...
TabMain:AddButton({
    Name = "Kill All", Description = "Elimina todos.",
...
Sec:AddMultiDropdown({ Name="Ignorar", Options={"Aliados","Inimigos","Neutros"},
    Default={"Aliados"}, Flag="Ignore", Callback=function(s) print(s) end })
Sec:AddKeybind({ Name="Toggle UI", Default=Enum.KeyCode.RightShift,
...
SecP:AddSlider({ Name="WalkSpeed", Description="Velocidade de movimento.",
    Min=16, Max=500, Default=16, Suffix=" st", Flag="WS",
    Callback=function(v)
        local c=game.Players.LocalPlayer.Character
        if c then c.Humanoid.WalkSpeed=v end
    end })
SecP:AddTextBox({ Name="Alvo", Description="Nome do jogador.", Placeholder="Player1",
    Flag="Target", Callback=function(t,e) if e then print(t) end end })

local SecV = TabVisual:AddSection({ Name = "ESP" })
SecV:AddToggle({ Name="ESP", Flag="ESP", Callback=function(s) print(s) end })
SecV:AddColorPicker({ Name="Cor ESP", Default=Color3.fromRGB(40,180,70),
    Flag="ESPColor", Callback=function(c) print(c) end })
SecV:AddSeparator()
SecV:AddLabel({ Text="Storm Hub v3.0", Color=Color3.fromRGB(40,180,70) })
