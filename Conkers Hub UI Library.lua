...
function ConkersHub:CreateWindow(config)
    ...
    local function buildTSnap()
        tSnap = {}
        -- Percorre TODOS os descendentes de Main de uma vez (GetDescendants é mais robusto)
        for _, inst in ipairs(Main:GetDescendants()) do
            -- Pula zonas proibidas
            if inForbiddenZone(inst) then
                -- skip

            -- TextLabel e TextBox: nunca afetar nada
            elseif inst:IsA("TextLabel") or inst:IsA("TextBox") then
                -- skip

            -- UIStroke: salva Transparency
            elseif inst:IsA("UIStroke") then
                tSnap[inst] = { stroke = inst.Transparency }

            -- ImageLabel / ImageButton: salva BackgroundTransparency + ImageTransparency
            elseif inst:IsA("ImageLabel") or inst:IsA("ImageButton") then
                tSnap[inst] = {
                    bg  = inst.BackgroundTransparency,
                    img = inst.ImageTransparency,
                }

            -- Frame / ScrollingFrame / TextButton: salva BackgroundTransparency
            elseif inst:IsA("Frame") or inst:IsA("ScrollingFrame")
                or inst:IsA("TextButton") then
