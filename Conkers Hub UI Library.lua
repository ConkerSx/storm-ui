    ...
    local function MakeComponents(container)
        ...
        function obj:AddColorPicker(cfg)
            ...
            UserInputService.InputEnded:Connect(function(i)
                ...
                or i.UserInputType == Enum.UserInputType.Touch then
                    boxDrag = false
                    task.defer(function() anyDragging = false end)
                end
            end)
            ...
            UserInputService.InputBegan:Connect(function(input)
                if not popOpen or anyDragging then return end
                if input.UserInputType ~= Enum.UserInputType.MouseButton1
                and input.UserInputType ~= Enum.UserInputType.Touch then return end
                ...
                if not ins(Popup) and not ins(Container) then ClosePopup() end
            end)
            ...
            CBtn.MouseButton1Click:Connect(function()
                ...
                Popup.Visible = true
                Tween(Popup, 0.22, {Size = UDim2.new(0, popupW, 0, POPUP_H)},
                    Enum.EasingStyle.Quart, Enum.EasingDirection.Out)
                -- Atualiza visual ao abrir (estado pode ter mudado via :Set())
                Apply()
            end)
            CBtn.MouseEnter:Connect(function() Tween(Container, 0.15, {BackgroundColor3 = Theme.ElementHover}) end)
            CBtn.MouseLeave:Connect(function() Tween(Container, 0.15, {BackgroundColor3 = Theme.Element}) end)
