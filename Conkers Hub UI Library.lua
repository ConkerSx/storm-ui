...
function ConkersHub:CreateWindow(config)
    ...
    function Window:Notify(cfg)
        ...
        local order = #notifySlots + 1
        table.insert(notifySlots, order)
        ...
        task.delay(duration, function()
            ...
            task.delay(0.3, function()
                Card:Destroy()
                table.remove(notifySlots, table.find(notifySlots, order) or 1)
            end)
        end)
    end

    -- SetTransparency: torna fundos da UI transparentes mantendo textos opacos.
    -- Valor 0 = sólido, 1 = invisível.
    function Window:SetTransparency(alpha)
        ...
        local function collect(inst)
            for _, child in ipairs(inst:GetChildren()) do
                ...
                if not isText and (child:IsA("Frame") or child:IsA("ScrollingFrame") or child:IsA("ImageLabel") or child:IsA("ImageButton")) then
    ...
    function Window:AddTab(tabConfig)
        ...
        function Tab:AddSection(sectionConfig)
            ...
            -- Label já existe como AddLabel, alias:
