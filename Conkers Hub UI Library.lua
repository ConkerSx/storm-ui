...
function ConkersHub:ThemeAll(name)
    ...
    for k, v in pairs(p) do Theme[k] = v end
    -- ThemeAll aplica ThemeRegistry (parcial) e ThemeAllRegistry (global absoluto)
    -- Ordem: primeiro ThemeRegistry (bases), depois ThemeAllRegistry (overrides/extras)
    ApplyThemeAll()
    ...
    local function MakeComponents(container)
        ...
        function obj:AddDropdown(cfg)
            ...
            local function OpenList()
                listOpen = true
                -- Centraliza o List dentro do Main via CentralOverlay
                local listH = math.min(#options * 30 + 8, 180)
                local mainW = Main.AbsoluteSize.X
                local listW = math.min(mainW - 32, 260)
                List.Parent = CentralOverlay
                List.Position = UDim2.new(0.5, -(listW / 2), 0.5, -(listH / 2))
            ...
            -- CloseOnOutsideClick substituído pelo CentralOverlay backdrop
        ...
        function obj:AddMultiDropdown(cfg)
            ...
            local function OpenMulti()
                listOpen = true
                local nOpts = 0; for _ in pairs(options) do nOpts = nOpts + 1 end
                if type(options) == "table" and #options > 0 then nOpts = #options end
        ...
        function obj:AddColorPicker(cfg)
            ...
            CBtn.MouseButton1Click:Connect(function()
                ...
                popOpen = true
                -- Centraliza no Main via CentralOverlay
                local mainW = Main.AbsoluteSize.X
                popupW = math.min(mainW - 32, 260)
                Popup.Parent = CentralOverlay
                Popup.Position = UDim2.new(0.5, -(popupW / 2), 0.5, -(POPUP_H / 2))
        ...
        function obj:AddSocials(cfg)
            ...
            -- Plataformas suportadas e suas configurações visuais padrão
            ...
            -- Filtra só as plataformas configuradas, na ordem canônica
            local active = {}
            ...
            -- Container externo (ocupa a largura do tab)
            ...
            -- Layout: 1-2 cards → coluna única grande; 3+ → grid 2 colunas
            local useGrid = (#active >= 3)
            local CARD_H  = useGrid and 72 or 82   -- altura do card
            local CARD_GAP = 6
            ...
            if useGrid then
                ...
                -- Auto-height com base no número de linhas
            ...
            for idx, entry in ipairs(active) do
                ...
                table.insert(cardThemeRefs, Card)

                -- Acento colorido à esquerda (barra vertical)
                ...
                Corner(CopyBtn, 5)
                local CopyStroke = Stroke(CopyBtn, icoColor, 1)
                RegisterTheme(CopyBtn, "BackgroundColor3", "Element")
                RegisterThemeAll(CopyBtn, "BackgroundColor3", "Element")
                ...
                -- Copy click: copia link e notifica
                CopyBtn.MouseButton1Click:Connect(function()
                    if link ~= "" then
                        pcall(function() setclipboard(link) end)
                        pcall(function() (game:GetService("GuiService") or {}).Clipboard = link end)
                    end
            ...
            -- Garante que Grid tem tamanho correto no layout de lista
            if not useGrid then
                GLy:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function()
                    Grid.Size = UDim2.new(1, 0, 0, GLy.AbsoluteContentSize.Y)
                end)
